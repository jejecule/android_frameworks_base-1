...
* Also Modifed by THE Metallium OS Project (C).
*
...
import com.android.systemui.bubbles.model.Contact;
import com.android.systemui.bubbles.animations.SimpleAnimtionListener;
import com.android.systemui.bubbles.model.ContactUtils;

public class ContactBubbleActivity extends Activity {

	public  static final String TAG = ContactBubbleActivity.class.getSimpleName();
	...
	public static Intent createIntent(Context context, View view, Contact contact) {
		int[] screenLocation = new int[2];
		view.getLocationOnScreen(screenLocation);
		return new Intent(context.getApplicationContext(), ContactBubbleActivity.class)
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContact  = intent.getParcelableExtra(EXTRA_CONTACT);
		
		mPopupLayout = new PopupLayout(this);
		setContentView(getLayoutInflater()
			.inflate(R.layout.contact_bubble, mPopupLayout));
		
		View contactBubble = mPopupLayout.findViewById(R.id.contact_bubble);
		contactBubble.setBackgroundDrawable(new PopupBackground(this));
		
		TextView contactName = (TextView)mPopupLayout.findViewById(R.id.contact_name);
		TextView contactStatus = (TextView)mPopupLayout.findViewById(R.id.contact_status);

		contactName.setText(mContact.getName());
		//contactName.setTextAppearance(this, android.R.style.TextAppearance_Small);

		contactStatus.setText(mContact.getStatus());
		contactStatus.setTextAppearance(this, android.R.style.TextAppearance_Small);
	...
	private class PopupBackground extends Drawable
	{
		...
		public PopupBackground(Context context)
		{
			...
			mCorner = res.getDimensionPixelSize(R.dimen.contact_bubble_corner);
			mPaint.setColor(res.getColor(R.color.contact_bubble_bg));
